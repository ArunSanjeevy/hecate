...
function buildHashInput(experimentKey, visitorId, salt) {
  return `${experimentKey}:${visitorId}:${salt || ''}`;
}
...
function assignVariant(visitorId, experiment) {
  ...
  const { key: experimentKey, salt, variants } = experiment;
  if (!variants || !Array.isArray(variants) || variants.length === 0) {
    return null;
  }

  const hashInput = buildHashInput(experimentKey, visitorId, salt);
